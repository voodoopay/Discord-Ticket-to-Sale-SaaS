...
vi.mock('@voodoo/core', () => {
  ...
  class SportsDataService {
    public async getPlayerDetails(): Promise<never> {
      throw new Error('Mock getPlayerDetails not implemented');
    }
  }
...
function createInteractionMock(query = 'James Tavernier'): {
...
describe('player command', () => {
  ...
  it('returns a player page style summary', async () => {
    ...
    vi.spyOn(SportsDataService.prototype, 'getPlayerDetails').mockResolvedValue(
      createOkResult({
        playerId: 'player-1',
        ...
        position: 'Defender',
        dateBorn: '1991-10-31',
        description: 'Club captain and attacking full-back.',
        imageUrl: null,
        cutoutUrl: null,
      }) as Awaited<ReturnType<SportsDataService['getPlayerDetails']>>,
    );
    ...
    expect(editReply).toHaveBeenCalledWith({
      content: expect.stringContaining('Player profile for'),
